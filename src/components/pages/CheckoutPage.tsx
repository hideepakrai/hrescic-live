      ...
      <div className="min-h-[80vh] flex flex-col items-center justify-center px-[5%] text-center">
        ...
        >
          ...
          <p className="text-muted font-semibold mb-10">
            Thank you for your purchase. Your order #NC-4921 has been placed and is being processed. We've sent a confirmation email to your inbox.
          </p>
          <Link 
            href="/" 
  ...
  return (
    <div className="pb-24 pt-12 px-[5%] max-w-7xl mx-auto">
      <div className="flex items-center gap-4 mb-12">
        <Link href="/cart" className="w-10 h-10 rounded-full border border-border flex items-center justify-center hover:bg-surface transition-colors">
          <ChevronLeft size={20} />
        </Link>
            ...
            {[
              ...
              <div key={s.id} className="flex items-center gap-3">
                <div className={`w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold transition-all ${
                  step >= s.id ? 'bg-secondary text-white' : 'bg-surface text-muted border border-border'
                }`}>
                ...
                {s.id < 3 && <div className="w-8 h-px bg-border hidden sm:block" />}
              </div>
            ))}
          </div>
            ...
            <AnimatePresence mode="wait">
                ...
                >
                  ...
                  <div className="grid sm:grid-cols-2 gap-6">
                    <div className="space-y-2">
                      <label className="text-[11px] font-black uppercase tracking-[2px] text-muted ml-1">First Name</label>
                      <input required type="text" className="w-full h-14 px-6 rounded-xl bg-surface border border-border outline-none focus:border-secondary transition-all font-semibold" />
                    </div>
                    <div className="space-y-2">
                      <label className="text-[11px] font-black uppercase tracking-[2px] text-muted ml-1">Last Name</label>
                      <input required type="text" className="w-full h-14 px-6 rounded-xl bg-surface border border-border outline-none focus:border-secondary transition-all font-semibold" />
                    </div>
                  </div>
                  <div className="space-y-2">
                    <label className="text-[11px] font-black uppercase tracking-[2px] text-muted ml-1">Address</label>
                    <input required type="text" className="w-full h-14 px-6 rounded-xl bg-surface border border-border outline-none focus:border-secondary transition-all font-semibold" />
                  </div>
                  <div className="grid sm:grid-cols-3 gap-6">
                    <div className="space-y-2">
                      <label className="text-[11px] font-black uppercase tracking-[2px] text-muted ml-1">City</label>
                      <input required type="text" className="w-full h-14 px-6 rounded-xl bg-surface border border-border outline-none focus:border-secondary transition-all font-semibold" />
                    </div>
                    <div className="space-y-2">
                      <label className="text-[11px] font-black uppercase tracking-[2px] text-muted ml-1">State</label>
                      <input required type="text" className="w-full h-14 px-6 rounded-xl bg-surface border border-border outline-none focus:border-secondary transition-all font-semibold" />
                    </div>
                    <div className="space-y-2">
                      <label className="text-[11px] font-black uppercase tracking-[2px] text-muted ml-1">ZIP Code</label>
                      <input required type="text" className="w-full h-14 px-6 rounded-xl bg-surface border border-border outline-none focus:border-secondary transition-all font-semibold" />
                    </div>
                  </div>
                </motion.div>
                ...
                >
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="p-6 rounded-2xl border border-border bg-surface flex items-center justify-between cursor-pointer hover:border-secondary transition-all">
                      <div className="flex items-center gap-4">
                        <div className="w-6 h-6 bg-blue-600 rounded-md flex items-center justify-center text-[10px] font-black text-white">UPI</div>
                  ...
                  <div className="space-y-4 pt-4">
                    <div className="space-y-2">
                      <label className="text-[11px] font-black uppercase tracking-[2px] text-muted ml-1">Card Number</label>
                      <input required type="text" placeholder="0000 0000 0000 0000" className="w-full h-14 px-6 rounded-xl bg-surface border border-border outline-none focus:border-secondary transition-all font-semibold" />
                    </div>
                    <div className="grid grid-cols-2 gap-6">
                      <div className="space-y-2">
                        <label className="text-[11px] font-black uppercase tracking-[2px] text-muted ml-1">Expiry Date</label>
                        <input required type="text" placeholder="MM/YY" className="w-full h-14 px-6 rounded-xl bg-surface border border-border outline-none focus:border-secondary transition-all font-semibold" />
                      </div>
                      <div className="space-y-2">
                        <label className="text-[11px] font-black uppercase tracking-[2px] text-muted ml-1">CVV</label>
                        <input required type="text" placeholder="123" className="w-full h-14 px-6 rounded-xl bg-surface border border-border outline-none focus:border-secondary transition-all font-semibold" />
                      </div>
                    </div>
                  </div>
                </motion.div>
              ...
              {step === 3 && (
                <motion.div 
                  ...
                  className="space-y-8"
                >
                  <h2 className="text-2xl font-bold tracking-tight mb-6">Review Order</h2>
                  <div className="p-8 rounded-3xl border border-border bg-surface space-y-6">
                    <div className="flex justify-between items-start">
                      <div>
                        <h4 className="text-[11px] font-black uppercase tracking-[2px] text-muted mb-2">Shipping To</h4>
                        <p className="font-bold">John Doe</p>
                        <p className="text-sm text-muted font-semibold">123 Design District, MG Road, Bangalore</p>
                      </div>
                      <button onClick={() => setStep(1)} className="text-secondary text-xs font-black uppercase tracking-wider">Edit</button>
                    </div>
                    <div className="h-px bg-border" />
                    <div className="flex justify-between items-start">
                      <div>
                        <h4 className="text-[11px] font-black uppercase tracking-[2px] text-muted mb-2">Payment Method</h4>
                        <p className="font-bold">Visa ending in 4242</p>
                      </div>
                      <button onClick={() => setStep(2)} className="text-secondary text-xs font-black uppercase tracking-wider">Edit</button>
                    </div>
                  </div>
                </motion.div>
              )}
            ...
            <div className="flex gap-4 pt-8">
              ...
              >
                {isProcessing ? (
                  ...
                  <>
                    {step === 3 ? 'Complete Order' : 'Continue to ' + (step === 1 ? 'Payment' : 'Review')}
                    <ArrowRight size={18} />
                  </>
                )}
              </button>
        ...
        <div className="space-y-8 sticky top-[140px]">
          ...
          <div className="p-6 rounded-2xl bg-emerald-50 border border-emerald-100 flex gap-4 items-start">
            ...
            <div>
              ...
              <p className="text-xs text-emerald-700 font-semibold mt-1">Your data is protected by 256-bit SSL encryption.</p>
            </div>
          </div>
        </div>
      </div>
